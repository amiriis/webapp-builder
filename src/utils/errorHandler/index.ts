...
const errorServer = (
  ...
  if (notification)
    NotificationManager(pushToastList, "warning", t, response.status);
};
const errorClient = (
  pushToastList: IPushToastList,
  response: any,
  ...
  switch (response.status) {
    case 401:
      clearToken();
      if (notification)
        NotificationManager(pushToastList, "error", t, response.status);
      break;
    case 422:
  ...
  if (notification) {
    if (Array.isArray(response.data.message)) {
      response.data.message.map((item: any) => {
        NotificationManager(pushToastList, "error", t, response.status, item);
      });
    } else
      NotificationManager(
        pushToastList,
